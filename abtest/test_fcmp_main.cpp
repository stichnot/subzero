...
int main(int argc, char **argv) {
  ...
  const static unsigned NumFloatValues = sizeof(FloatValues) / sizeof(*FloatValues);
  ...
  const static unsigned NumDoubleValues = sizeof(DoubleValues) / sizeof(*DoubleValues);
  ...
  const static unsigned NumFuncDouble = sizeof(FuncDouble) / sizeof(*FuncDouble);
      ...
      for (unsigned j = 0; j < NumFloatValues; ++j) {
        ...
        if (ResultSz == ResultLlc) {
          ...
          printf("%s(%g, %g): sz=%d llc=%d\n",
                 FuncFloat[f].Name, FloatValues[i], FloatValues[j], ResultSz, ResultLlc);
        }
      }
    }
  }
    ...
    for (unsigned i = 0; i < NumDoubleValues; ++i) {
      for (unsigned j = 0; j < NumDoubleValues; ++j) {
        ...
        if (ResultSz == ResultLlc) {
          ...
          ++Failures;
          printf("%s(%g, %g): sz=%d llc=%d\n",
                 FuncDouble[f].Name, DoubleValues[i], DoubleValues[j], ResultSz, ResultLlc);
        }
      }
    }
  }
